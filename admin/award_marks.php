...
*/ 
 ...
 $sql="insert into images_award(id,theme_id,user_id,name,new_name,title,active,status,modified)(select id,theme_id,user_id,name,new_name,title,active,status,modified from images where images.id in (select timgid from temp_award where tcrctid = '$circuit' and tsecid = '$section'))";
 $mysqldb->query($sql);
  
  $sql = "update images_award set circuit_id = '$circuit' where circuit_id = 0";
  $mysqldb->query($sql);
