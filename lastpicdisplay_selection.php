<?php
	...
	if(!empty($ses)){
		...
		$sql = "select image_srl_no, image_id_actual, selection_marks 
					from selection_marks_view 
					where ccode = '".$_SESSION['circuit']."'
					and section_code = '".$_GET['section']."'
					and judge_code = '".$_SESSION['judge']."' and selection_marks > 0 order by id";
				//echo $sql;
		...
		if($result->num_rows > 0)
		{
			while($row = $result->fetch_array())
			{
				...
				$selection_marks = $row['selection_marks'];
			}
	...
	if(!empty($foto_no) AND $mflag == 1)
	{
			...
			if(!$rows > 0)
			{
				$query = 'create view selection_marks_view as 
					select ccode, section_code, judge_code, image_srl_no, image_id_actual, selection_marks 
					from selection_marks_table inner join circuit_master on circuit_master.id = selection_marks_table.circuit_id 
					inner join section_list on section_list.section_id = selection_marks_table.section_id 
					inner join judge_master on judge_master.id = selection_marks_table.judge_id';
				//echo $query;
				$result = $mysqldb->query($query);
				$flag = 1;
			}
			...
			if($flag == 1)
			{
				$query = 'select selection_marks,image_id_actual from selection_marks_view 
			...
			$judge_marks[] = array('judge_code'=>$_SESSION['judge'],'selection_marks'=>$selection_marks,'prev_image'=>$prev,'next_image'=>$next,'present_image'=>$foto_no,'actual_imgid'=>$present_actual, 'new_name'=>$new_name, 'flag'=>"$imflag");
