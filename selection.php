<?php
	...
	if(isset($_SESSION['judge']) AND isset($_SESSION['circuit'])){
		...
		$sql = "select btn_value from button_details where btn_type = 1 order by id";
		$result = $mysqldb->query($sql);
...
#get_photo {width:800px; height:500px; margin:1% auto; border-radius:15px; -moz-border-radius:15px; border:0px solid yellow; background:none; line-height:220px; font-size:300%; text-align:center; color:blue; clear:both;} /*#fefefe*/
	...
	$(function(){
		...
		$('#getsession').change(function(){
			...
			if(section != ''){
				$.ajax({
					type:'get',
					url:'foto_no_display.php?section='+section,
					dataType:'json',
					success:function(data){
						$.each(data, function(i,j){
							var section = 'Theme : '+j.section_name+'';
							...
							$('#disp').empty().append(j.selection_marks);
		...
		var get_last_marked_image = function(){
			...
			$.ajax({
				type:'get',
				url:'lastpicdisplay_selection.php',
				data:{
					'section':section
				},
				dataType:'json',
